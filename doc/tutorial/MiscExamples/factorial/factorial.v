...
Theorem factorial_correct : forall ge e m n le,
    ...
    le!_input = Some (Vint_of_nat n) ->
    exists t le' out, exec_stmt ge e le m f_factorial.(fn_body) t le' m out
...
Theorem factorial_correct_weak : forall ge e m n f le,
    Z.of_nat n < Int.modulus ->
    Z.of_nat (fact n) < Int.modulus ->
    fact n = f -> 
    le!_input = Some (Vint_of_nat n) -> exists t le' out, exec_stmt ge e le m f_factorial.(fn_body) t le' m out /\ (le'!_output) = Some (Vint_of_nat f).
  Proof.
    intros.
    rewrite <- H1.    
    assert (exists t le', exec_stmt ge e (PTree.set _output (Vint_of_nat 1) le) m factorial_loop t le' m Out_normal /\
    ...
    eexists. eexists. exists (Out_return (Some (Vint_of_nat (fact n), tuint))). split.
    + eapply exec_Sseq_1. econstructor. econstructor. eapply exec_Sseq_1. apply H2. repeat econstructor. rewrite Nat.mul_1_r in H2. apply H2.
    + rewrite Nat.mul_1_r in H2. apply H2.
Qed.

  Theorem factorial_correct_corr : (forall ge e m n f le,
    Z.of_nat n < Int.modulus ->
    Z.of_nat (fact n) < Int.modulus ->
    fact n = f -> 
    le!_input = Some (Vint_of_nat n) -> exists t le' out, exec_stmt ge e le m f_factorial.(fn_body) t le' m out /\ (le'!_output) = Some (Vint_of_nat f))
