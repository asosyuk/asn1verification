...
\item[Task 4] Proof of High-Level Properties 

\item[Task 5] Code Extraction from Execution Specification

\item[Task 6] Final Code, Associated Documentation, Estimates of Additional Work Items
 \end{description}

\section{Project Status}

Now, we are working in parallel on the high-level specification of the standard and executable specification of the \texttt{asn1c}
compiler, the intermediate level between the high-level specification
and the C code. We start by formalizing BER and DER encoding rules for several primitive and constructed types in relational style. Each premise of a constructor formalizes (part of) a paragraph in the standard. E.g., for boolean type:
...
Then we use our primitive types specifications to formalize DER relation for each type using predicates about length and tags. The formalization is first-order so ideally it could be easily translated to other languages than Coq and the relation to the text is straightforward.
...
As for the executable specification, there are many ways of approaching this, but what we
have to keep in mind is the future proof effort, especially with respect to
the C code. Thus, we write executable specification that is close to the actual implementation. 

The \texttt{asn1c} compiler has a modular structure, so we proceed with
verification in a modular way and exploit this structure in the
proofs. From a high-level view, the \texttt{asn1c} compiler consists of
...
of this. We started with the simplest types in this way: we wrote the high-level specification that corresponds to the C function and then checked it with respect to the specification. Even in the simplest boolean type we found that the decoder was behaving not accroding to the standard (in particular, several encodings for boolean values were possible which clearly violated the ASN.1 standard).
...
given ASN.1 type, the \texttt{asn1c} compiler produces an internal
