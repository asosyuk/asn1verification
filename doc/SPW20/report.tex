...
As a ``first-order'' estimate of the difficluties associated with
verifying ASN.1-related programs, we made an attempt at writing a
formal specification of an encoder-decoder pair for a small, but
particularly error-prone subset of the standard - floating-point numbers \cite{TODO:#?<asn1fpcoq>}.
...
% TODO: make me a list please
* Writing a new implementation in Coq does not produce any improvement in pre-existing widely-used implementations
* Automatic code extraction does not provide sufficient correctness guarantees
* Extracted code generally runs much slower than its counterparts implemented in other languages
* Extracted code might not be compatible with or viable in some real-world use-case scenarios
% /TODO
