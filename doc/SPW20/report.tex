...
\begin{abstract}
  ...
  formal verification will be layered atop a tested ASN.1
...
prove that the semantics of Clight
translation for each given function correspond to that function's VST specification. This guarantees that the C program terminates, is memory safe, and behaves exactly like the executable and hence the high-level specification.
...
Then, we use our primitive type specifications to formalize DER relations for each type using predicates about length and tags. The formalization is first-order, so it can easily be translated to languages other than Coq. Moreover, the relation to the text is straightforward, which will ease inspection and validation.
...
While Digamma.ai is committed to sponsoring the initial stage of the
