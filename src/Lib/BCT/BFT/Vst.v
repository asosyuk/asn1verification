...
Definition ber_fetch_tag_spec : ident * funspec :=
  DECLARE _ber_fetch_tag
    WITH ptr_p : val, ptr_v : list Z, size : Z, 
         tag_p : val, tag_v : Z
    PRE [tptr tvoid, tuint, tptr tuint]
      PROP ((*0 <= size <= Int.max_unsigned;
            ...
            isptr tag_p *))
      PARAMS (ptr_p; Vint (Int.repr size); tag_p)
      GLOBALS ()
      SEP ((* data_at Tsh (tarray tuchar (Zlength ptr_v)) 
                   (map (fun v => Vubyte (Byte.repr v)) ptr_v) ptr_p;
           data_at Tsh tuint (Vubyte (Byte.repr tag_v)) tag_p;
           valid_pointer tag_p *))
    POST [tint]
    (*  let r := ber_fetch_tags ptr_v 0 size tag_v (sizeof tuint) in *)
      PROP ()
      LOCAL (temp ret_temp
                  (Vint (Int.repr 0)))
      SEP ((*data_at Tsh (tarray tuchar (Zlength ptr_v)) 
