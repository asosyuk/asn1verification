...
Definition length_serialize len size : errW1 asn_enc_rval :=
   if (127 >=? Int.signed len)%Z then
    if eq_dec size 0%int 
    then raise (CustomError DWT_Error)
    else tell1 [Int.zero_ext 8 len] ;; ret (encode 1%Z) 
  else let r := required_size len in 
       if (r <? Int.unsigned size)%Z 
       then tell1 (serialize_length_app len) ;; ret (encode (r + 1)%Z) 
       else raise (CustomError DWT_Error).

Lemma length_serialize_req_size : forall l s ls z, 
    length_serialize l s [] = inr (ls, encode z) ->
    len ls = z.
Proof.
intros until z.
